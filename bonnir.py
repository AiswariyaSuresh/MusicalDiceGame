...
def minuet_filename(mmid):
    name1 = str(mmid)
    minuefilename = "M" + name1 + ".wav"
    return minuefilename
...
def construct_waltz():
    minuetlist = []
    for x in minuet_table:
        rfm = roll_dice(2)
        minuetlist.append(minuet_filename(x[rfm]))
    triolist = []
    for x in trio_table:
        rft = roll_dice(1)
        triolist.append(trio_filename(x[rft]))
    waltzlist = minuetlist + triolist
    for z in minuetlist:
        wo = simpleaudio.WaveObject.from_wave_file(z)
        player = wo.play()
        player.wait_done()
